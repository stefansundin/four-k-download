    ...
    m_srcObject(NULL)
{
    ...
    m_itemsBinding = QSharedPointer<Binding>(new Binding(this, "items", m_srcObject, m_itemsProperty));
    if (!m_iconsProperty.isEmpty())
        m_iconsBinding = QSharedPointer<Binding>(new Binding(this, "icons", m_srcObject, m_iconsProperty));
    m_indexBinding = QSharedPointer<Binding>(new Binding(m_combobox, "currentIndex", m_srcObject, indexProperty, Binding::TwoWay));
}
