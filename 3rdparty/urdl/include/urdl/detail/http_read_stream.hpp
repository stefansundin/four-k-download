...
public:
  ...
  boost::system::error_code open(const url& u, boost::system::error_code& ec)
  {
    ...
    std::string additional_headers
      = options_.get_option<urdl::http::additional_headers>().str();
    std::string connection_type
      = options_.get_option<urdl::http::connection_type>().value();
    ...
    if (additional_headers.length())
        request_stream << additional_headers;

    if (!connection_type.length())
        request_stream << "\r\n";
    else
    {
        request_stream << "Connection: " << connection_type << "\r\n\r\n";
    }
