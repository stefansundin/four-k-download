...
void MainView::setViewModel(QObject* value)
{
    if (m_viewModel.data() != value)
    {
        if (m_viewModel)
        {
            ...
            ui->toolBar->removeAction(m_viewModel.data()->browseAction());
            ...
            removeAction(m_viewModel.data()->browseAction());
        ...
        if (m_viewModel && m_factory)
        {
            ...
            ui->toolBar->addAction(m_viewModel.data()->browseAction());
            ...
            addAction(m_viewModel.data()->browseAction());
...
{
    return ui->btnAction->width() + ui->btnRemove->width() + 2 * BorderOffset;
}
