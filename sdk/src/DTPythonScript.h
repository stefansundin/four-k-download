/// Copyright 2010-2012 4kdownload.com (developers@4kdownload.com)
/**
    This file is part of 4k Download.

    4k Download is free software; you can redistribute it and/or modify
    it under the terms of the one of two licenses as you choose:

    1. GNU GENERAL PUBLIC LICENSE Version 3
    (See file COPYING.GPLv3 for details).

    2. 4k Download Commercial License
    (Send request to developers@4kdownload.com for details).
   
*/


/// \file   DTPythonScript.h

#ifndef _DTPYTHONSCRIPT_H_INCLUDED_
#define _DTPYTHONSCRIPT_H_INCLUDED_

#if defined(_MSC_VER) && (_MSC_VER >= 1020)
#pragma once
#endif

#include <openmedia/DTTypes.h>
#include <openmedia/DTDeclareImpl.h>
#include <openmedia/DTCommon.h>

#include <openmedia/DTScript.h>

namespace openmedia { namespace script { namespace python {

class python_script : public script
{
public:
    python_script(const ::std::string & PythonScript);
};

} } }

#endif //#ifndef _DTPYTHONSCRIPT_H_INCLUDED_
